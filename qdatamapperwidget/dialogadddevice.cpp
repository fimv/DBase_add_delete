 ...
 * -------------------------------------------------------------------------------------------------
 * */
void DialogAddDevice::on_SeekButton_clicked()
{
    ...
    QSqlQuery query;
    QString str = QString("(SELECT " DEVICE_HOSTNAME " FROM " DEVICE
                          " WHERE ( " DEVICE_HOSTNAME " = '%1') "
                          " OR " DEVICE_IP " = '%2' "
                           " OR " DEVICE_MAC " = '%3'"
                          " AND id NOT LIKE '%0' )")
    ...
    /* Если запись не существует, то диалог вызывает
     * предупредительное сообщение
     * */
    if(query.value(0) != 0){
      mapper->toPrevious();
      ...
      model->submitAll();
        ...
        QMessageBox::information(this, tr("Ошибка хоста"),
                                 tr("Хост с таким именем или IP-адресом не существует"));
    }
}
 ...
 * -------------------------------------------------------------------------------------------------
 * */
void DialogAddDevice::on_DeleteButton_clicked()
{
    mapper->toPrevious();
    mapper->toNext();
